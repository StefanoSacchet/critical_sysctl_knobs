...
\end{enumerate}
However, the efficiency of this loop is highly dependent on the quality of the initial knob selection.
An example of autotuning architecture can be found in Figure \ref{fig:autotuning_loop}.

% \begin{figure}
%     \centering
%     \includesvg[width=1\linewidth]{images/2_background/knob_autotuining_loop}
%     \caption{High-level architecture of a typical autotuning framework. The system is divided into a knob identification phase, which filters the search space to find critical parameters, and an online autotuning loop, which iteratively optimizes these parameters using feedback from the target system.}
