...
When invoked, they traverse the VFS layer before being dynamically routed to specific file system drivers or socket implementations via the \texttt{struct file\_operations} indirect calls. Without context-sensitive pruning, the static analysis graph for \texttt{write} branches out into every block-device driver, file system (e.g., ext4, btrfs, XFS), and socket protocol capable of transmitting data, resulting in a massive baseline of 6190 nodes.

Interestingly, the baseline node count for the \texttt{read} system call was observed to be slightly lower starting at 5,223 nodes.
...
The impact on the graph size is dramatic: the write graph is reduced from 6,190 down to just 228 nodes (a 96.3\% reduction). Similarly, the read graph shrinks from 5,223 down to 114 nodes (a 97.8\% reduction).
...
The baseline graphs for these system calls are highly complex because the kernel must iterate through generic file descriptors and invoke their respective poll callback functions, which could belong to sockets, pipes, character devices, or \textit{eventfd} constructs.
Without context-sensitive pruning, this generic routing results in massive static analysis graphs: \texttt{epoll\_wait} begins with 4,192 nodes, while \texttt{poll} starts at 3,101 nodes.

By establishing a TCP networking context, the Syscall Analyser aggressively prunes these generic polling mechanisms down to the specific \texttt{tcp\_poll} implementations.
This successfully removes all irrelevant polling routines for non-network devices, yielding an impressive reduction in graph complexity.
The \texttt{epoll\_wait} graph collapses from 4,192 down to just 17 nodes (a 99.6\% reduction).
The poll graph shrinks from 3,101 down to 6 nodes (a 99.8\% reduction).
Ultimately, this pruning strategy yields a tight, accurate intersection target, isolating only the kernel paths that directly govern network event multiplexing.

\subsection{Verification of Pruning Accuracy}

Although achieving a high node reduction percentage ($P_{reduction}$) is necessary to solve the dimensionality problem, it introduces the critical risk of over-approximation—specifically, erroneously pruning execution paths that the application actively requires (false negatives).
To empirically validate the safety and precision of the pruned graphs, we applied the dynamic tracing methodology outlined in Section~\ref{subsec:methodology_dynamic_tracing}.

By executing the target workloads and dynamically tracing the live function call stacks using kernel tracing tools, we established a ground-truth execution path for the evaluated system calls.
For instance, Figure \ref{fig:sysread_flame} illustrates the flame graph generated during live execution of the read system call under a standard TCP/IPv4 networking workload.
The visualization captures the exact, real-world call stack transitioning from the Virtual File System (VFS) layer through the TCP receive mechanisms (e.g., \texttt{tcp\_recvmsg}).

When mapping the dynamically observed, ground-truth call stacks back onto the statically pruned graphs, the results demonstrated a high degree of structural fidelity along the critical execution paths.
While an absolute 100\% node-for-node overlap is practically unattainable in this context, this discrepancy is an expected artifact of the compilation and instrumentation toolchains rather than a flaw in the pruning logic.
Specifically, minor deviations arise primarily from compiler optimizations (such as aggressive function inlining, which merges function bodies and eliminates distinct stack frames in the compiled binary), as well as the inherent resolution limits and sampling intervals of dynamic tracing frameworks.
However, these missing nodes represent trivial, low-level wrapper functions.
Crucially, all semantically significant functions dynamically invoked by the kernel—those responsible for core subsystem routing, protocol state management, and the actual evaluation of sysctl parameters—were successfully located within the reduced static graphs.

This verification was consistent across the evaluated system calls.
It conclusively proves that the context-sensitive pruning strategy is highly precise, it aggressively eliminates thousands of irrelevant nodes (such as extraneous file systems and block device drivers) without sacrificing the structural completeness required to accurately discover influential \textit{sysctl} parameters.
...
The results of the intersection phase provide a powerful validation of the underlying methodology.
The framework successfully extracted highly influential tuning parameters, such as \texttt{tcp\_rmem}, \texttt{tcp\_wmem}, and \texttt{tcp\_congestion\_control}.
While these knobs are universally recognized by system administrators as critical for network performance, their extraction here represents a significant methodological milestone, because they emerged entirely automatically from the static analysis pipeline.
The system possessed no prior domain knowledge, heuristics, or pre-programmed network expertise.
By purely tracking control flow and data dependencies from a generic system call down through the kernel source code, the intersection organically converged on the exact parameters that human experts traditionally identify only through domain experience and manual trial-and-error.

Furthermore, the presence of parameters like \texttt{vm.swappiness} and various CPU scheduler configurations in the final subset highlights the strength of this automated, source-code-driven approach.
The static analysis pipeline accurately mapped the deep, cross-subsystem dependency chains, tracing a simple read or write system call directly into the kernel memory management and task scheduling routines.

\section{Evaluation Environment}

To empirically validate the performance impact of the critical \textit{sysctl} knobs identified by the intersection methodology, we deployed an automated benchmarking environment.
It is important to reiterate that the experiment execution pipeline and the machine learning optimizer are based on two existing external frameworks: \textit{metric-monitoring} and \textit{autotune}.
These tools are not original contributions of this thesis, but were utilized strictly as a test harness to validate the static analysis results.
Specifically, the metric-monitoring framework manages the practical mechanics of the benchmarking lifecycle.
It is responsible for applying the selected kernel configurations to the target virtual machines, executing the load generators (such as \textit{wrk}), and collecting the resulting performance telemetry.
Conversely, the autotune framework provides the Bayesian Optimization search algorithm that drives the experiment.

By feeding the statically derived list of intersected \textit{sysctl} knobs (detailed in Table~\ref{tab:critical_knobs}) into autotune as its strictly bounded search space, the system iteratively applied configurations and measured the resulting application throughput and latency.
This approach effectively transitions the research from theoretical static analysis to practical, runtime validation, definitively proving which of our statically flagged knobs are critical for maximizing performance under active load.
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Architecture diagram of the end-to-end automated benchmark orchestration and metric monitoring framework. It illustrates the flow from \textit{Ansible} orchestration to the target environment, highlighting load generation from the host machine and metric extraction to central storage. Note that this framework is not part of this thesis contribution.}
    \label{fig:metric_monitoring_architecture}
\end{figure}

\subsection{The Autotune Framework}

As introduced in Section~\ref{subsec:knob_relevanve_verification}, the empirical validation of the identified \textit{sysctl} knobs was automated using \textbf{Autotune}, an existing, external tuning framework.
It must be stated explicitly that the Autotune framework is not an original contribution of this thesis.
Rather, it serves strictly as an evaluation instrument to measure the efficacy of the static analysis pipeline.
...
\begin{figure}[htbp]
    ...
    \caption{Workflow of the Autotune framework. The Bayesian Optimization engine iteratively explores \textit{sysctl} configurations by interacting with the underlying metric-monitoring framework. The resulting history log is then post-processed to generate visual analytics. Note that this framework is not part of this thesis contribution.}
...
The evaluation of NGINX focused on its asynchronous and event-driven capabilities when subjected to the default $\sim 100kB$ payload workload (\textit{workload B}~\ref{subsec:methodology_workload}).
...
While the empirical evaluation demonstrates the efficacy of the proposed static analysis pipeline in reducing the configuration search space and accelerating automated tuning, several threats to validity must be acknowledged to properly contextualize the results.

\subsection{Internal Validity}

Internal validity concerns the factors within the experimental setup that could have influenced the measured outcomes.

\begin{itemize}
    \item \textbf{Dependence on the autotuner implementation}: the final throughput gains reported in this evaluation is inherently tied to the efficiency of the external Autotune framework. Utilizing a different optimization algorithm (such as Reinforcement Learning or Genetic Algorithms) could settle on slightly different local optima within the bounded search space.
    \item Virtualized \textbf{environment overhead}: the benchmarks were executed within virtual machines rather than on bare-metal hardware. Virtualization introduces a hypervisor abstraction layer, particularly within the network stack and interrupt handling subsystems. While this accurately reflects modern cloud deployments, hypervisor scheduling noise could introduce slight variances in metrics that would not be present in a bare-metal environment.
\end{itemize}

\subsection{External Validity}

External validity concerns the extent to which these findings can be generalized to other systems, environments, or workloads.
\begin{itemize}
    \item \textbf{Kernel version specificity}: the empirical evaluation and the resulting graph pruning metrics presented in this thesis were validated exclusively against a single, modern version of the Linux kernel (v6.10.11). Because the Linux kernel is a rapidly evolving project where internal data structures, and subsystem routing paths are frequently refactored, the exact set of 92 extracted networking knobs is inherently tied to this release. However, because the underlying framework was not strictly hardcoded to version 6.10.11, the foundational methodology of intersecting control and data flows remains broadly applicable. Applying this pipeline to legacy or future kernel releases would likely require only minor adjustments to the tooling, though it will naturally yield a different subset of parameters.
    \item \textbf{Limited workload scope}: the evaluation strictly targeted web-server workloads (Apache, NGINX, and HAProxy) handling synthetic HTTP traffic over TCP/IPv4. While this effectively proved the methodology for the networking subsystem, it does not guarantee identical tuning margins for radically different application profiles. Heavy relational databases (e.g., PostgreSQL) or distributed message queues (e.g., Kafka) heavily exercise the Virtual File System (VFS), block device layers, and inter-process communication (IPC) subsystems, which would require analysing different system calls and validating a completely different subset of \textit{sysctl} parameters.
\end{itemize}
